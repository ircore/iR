...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\iRcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\iRcoin
    // Mac: ~/Library/Application Support/iRcoin
    // Unix: ~/.iRcoin
#ifdef WIN32
    // Windows
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                if (!fMatch)
                {
                    ...
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("iRcoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
