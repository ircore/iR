...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a iRcoin address"));
    ...
    aboutAction = new QAction(QIcon(":/icons/bitcoin"), tr("&About iRcoin"), this);
    ...
    secondAuthAction->setStatusTip(tr("Second auth with your iRcoin addresses"));
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("iRcoin client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid iRcoin address or malformed URI parameters."));
    }
